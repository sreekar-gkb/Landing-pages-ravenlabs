...
export default function LeadForm() {
  ...
  async function onSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    if (result.ok) {
      ...
      window.location.href = '/zoho-zia-agents/thanks'
    } else {
      setStatus('error')
  ...
  return (
    <form onSubmit={onSubmit} noValidate>
      <div style={{ display: 'grid', gap: 'var(--rl-space-4)' }}>
        ...
        <label>What would you like to automate? <span>(optional)</span><textarea name="message" rows={3} /></label>
        <input name="website" tabIndex={-1} autoComplete="off" aria-hidden="true" style={{ position: 'absolute', left: '-10000px', opacity: 0 }} />
        {status === 'error' && <p role="alert" style={{ color: '#b42318', margin: 0 }}>{error}</p>}
        <button className="rl-btn-primary" type="submit" disabled={status === 'submitting'}>
